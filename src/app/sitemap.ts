...
export default function sitemap(): MetadataRoute.Sitemap {
  const staticRoutes = ["","/about","/services","/packages","/process","/portfolio","/growth-support","/testimonials","/faq","/resources","/contact","/lab","/medical-professionals","/cookie-policy"];
  const staticEntries: MetadataRoute.Sitemap = staticRoutes.map((path,index)=>({
    ...
    priority:index===0?1:path==="/medical-professionals"?0.95:path==="/contact"?0.9:path==="/cookie-policy"?0.5:0.8,
  }));
